...
PLOT_TITLE = 'Monitor de datos\n' #Nuevo
X_LABEL = 'Tiempo (x' + str(SAMPLING_RATE_MS) + ' ms)' #Nuevo
Y_LABEL= 'Magnitudes medidas (%)'  #Nuevo
#Define here the variables recieved from the Arduino with a text label and choice a RGB color for each one.
VARIABLES = [
				['Respuesta del Sistema', (1, 1, 0)],
				['Señal de Control', (0, 0, 0.75)],
				['Modelo de Referencia', (1, 0, 0.7)],
				['Error Sistema-Modelo', (1, 0.5, 0)]
			]
...
from Arduino_Monitor_Random import SerialData as DataGen
...
class dataseries(wx.Panel):
	def __init__(self, parent, ID, label, initval, color):
		
		wx.Panel.__init__(self, parent, ID)
		
		self.IsChecked = initval
		self.color = color
		self.label = label
		self.value = [0.0]
		self.line = 0
		
		self.cb_caption = wx.CheckBox(self, -1, label, style=wx.ALIGN_RIGHT)
		self.Bind(wx.EVT_CHECKBOX, self.check, self.cb_caption) 
		self.cb_caption.SetValue(True)
...
class GraphFrame(wx.Frame):
	...
	def create_main_panel(self):
		...
		#Let to create a new object for each monitored viariable defined in the array VARIABLES.
		self.data.append(dataseries(self.panel, -1, 'SetPoint', True, (0, 1, 0)))
	
		for var in VARIABLES:
			self.data.append(dataseries(self.panel, -1, var[0], True, var[1]))
		...
		self.cb_grid = wx.CheckBox(self.panel, -1, 
			"Mostrar Cuadrícula", style=wx.ALIGN_RIGHT)
		self.Bind(wx.EVT_CHECKBOX, self.on_cb_grid, self.cb_grid)
		self.cb_grid.SetValue(True)
		self.cb_xlab = wx.CheckBox(self.panel, -1, 
			"Mostrar Etiquetas en X", style=wx.ALIGN_RIGHT)
		self.Bind(wx.EVT_CHECKBOX, self.on_cb_xlab, self.cb_xlab)
		self.cb_xlab.SetValue(True)
		
		self.captionsbox = wx.StaticBox(self.panel, -1, "Mostrar:") #, size=(270, 540))
		self.captionsgroupsizer = wx.StaticBoxSizer(self.captionsbox, wx.VERTICAL)
		self.groupbox= wx.BoxSizer(wx.VERTICAL)
		for n in range(len(self.data)):
			self.groupbox.Add(self.data[n], 0, wx.ALL|wx.LEFT, 5)
			#self.captionsgroupsizer.Add(self.data[n], 0, wx.CENTER, 5)
		self.captionsgroupsizer.Add(self.groupbox,  0, wx.ALL|wx.LEFT, 5)
		
		self.hbox0 = wx.BoxSizer(wx.HORIZONTAL)
		self.hbox0.AddSpacer(10)
		self.hbox0.Add(self.canvas, 1, flag=wx.ALL | wx.LEFT | wx.TOP | wx.GROW)
		self.hbox0.Add(self.captionsgroupsizer, border=10, flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL)
		
		self.show = wx.BoxSizer(wx.VERTICAL)
		self.show.Add(self.cb_grid, border=0, flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL)
		self.show.Add(self.cb_xlab, border=0, flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL)

		self.hbox2 = wx.BoxSizer(wx.HORIZONTAL)
		self.hbox2.Add(self.pause_button, border=10, flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL | wx.GROW)
		self.hbox2.Add(self.setpoint, border=10, flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL)
		#self.hbox2.AddSpacer(20)
		self.hbox2.Add(self.show, 0, flag=wx.ALIGN_LEFT | wx.ALIGN_CENTER_VERTICAL) #Nuevo
		#self.hbox2.AddSpacer(20)
		self.hbox2.Add(self.xmin_control, border=5, flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL)
		self.hbox2.Add(self.xmax_control, border=5, flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL)
		#self.hbox2.AddSpacer(20)
		self.hbox2.Add(self.ymin_control, border=5, flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL)
		self.hbox2.Add(self.ymax_control, border=5, flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL)

		self.vbox = wx.BoxSizer(wx.VERTICAL)
		self.vbox.Add(self.hbox0, 1, flag=wx.ALL | wx.LEFT | wx.TOP | wx.GROW)
		#self.vbox.Add(self.canvas, 1, flag=wx.LEFT | wx.TOP | wx.GROW) #Obsoleto
		#~ self.vbox.Add(self.hbox1, 0, flag=wx.ALIGN_LEFT | wx.TOP)  #Obsoleto
		self.vbox.Add(self.hbox2, 0, flag=wx.CENTER | wx.TOP)
	...
	def init_plot(self):
		...
		self.fig.set_facecolor('0.07') #beb7b4 -> Oxygen KDE color
		
		self.axes = self.fig.add_subplot(111)
		self.axes.set_axis_bgcolor('0.15')
		self.axes.tick_params(axis='both', colors='0.7',color='0.7')
		self.axes.set_title(PLOT_TITLE.decode('utf-8'), size=14, color=(color_text))

		
		pylab.setp(self.axes.set_xlabel(X_LABEL.decode('utf-8')), fontsize=10, color=(color_text))
		pylab.setp(self.axes.set_ylabel(Y_LABEL.decode('utf-8')), fontsize=10, color=(color_text))
		pylab.setp(self.axes.get_xticklabels(), fontsize=8, color=(color_text))
		pylab.setp(self.axes.get_yticklabels(), fontsize=8, color=(color_text))
		...
		for n in range(len(self.data)):
			line = self.axes.plot(								#Nuevo
					self.data[0].value,									#Nuevo
					linewidth=1,								#Nuevo
					color=self.data[n].color,					#Nuevo
					label=self.data[n].label.decode('utf-8')	#Nuevo
					)[0]
			self.lineseries.append(line)
		
		leg = self.axes.legend(bbox_to_anchor=(1.0, 0.5), loc=3, frameon=False, labelspacing=1.0, prop={'size':9})
	...
	def draw_plot(self):
		...
		if self.xmax_control.is_auto():
			xmax = len(self.data[0].value) if len(self.data[0].value) > AUTO_RANGE else AUTO_RANGE
		else:
		...
		if self.ymin_control.is_auto():
			ymin = round(min(self.data[0].value), 0) - 1
			for n in range(len(self.data)):
				if (min(self.data[n].value) < (ymin + 1)):
					ymin = round(min(self.data[0].value), 0) - 1
		else:
			ymin = int(self.ymin_control.manual_value())

		if self.ymax_control.is_auto():
			ymax = round(max(self.data[0].value), 0) + 1
			for n in range(len(self.data)):
				if (max(self.data[n].value) > (ymax - 1)):
					ymax = round(max(self.data[0].value), 0) + 1
		else:
		...
		for x in range(len(self.data)):
			if self.data[x].IsChecked:
				self.lineseries[x].set_xdata(np.arange(len(self.data[x].value)))
				self.lineseries[x].set_ydata(np.array(self.data[x].value))
			else:
				self.lineseries[x].set_xdata([0])
				self.lineseries[x].set_ydata([0])
	...
	def on_save_data(self, event):
		...
		if dlg.ShowModal() == wx.ID_OK:
			...
			with open(path, 'w') as fp:
				...
				headers = []
				headers.append("Tiempo (ms)")
				for n in range(len(self.data)):
					headers.append(self.data[n].label)
				a.writerows([headers])
				
				time = array('i',[])
				for k in range(len(self.data[0].value)):
					time.append(SAMPLING_RATE_MS*k)
					
				col = []
				col.append(time)
				for n in range(len(self.data)):
					col.append(array('f',np.array(self.data[n].value)))	
				
				for k in range(len(self.data[0].value)):
					a.writerows([[col[i][k] for i in range(len(col))]])
	...
	def on_sampling_timer(self, event):
		if not self.paused:
			...
			self.data[0].value.append(self.setpoint.step_value())	#Nuevo
			for n in range(len(self.data)-1):
				self.data[n+1].value.append(self.ArduinoState[n])	#Nuevo
