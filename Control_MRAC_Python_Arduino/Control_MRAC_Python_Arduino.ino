...
//-------------------------------------CONTROL MRAC-------------------------------------------------------
  ...
  // y límites de escalamiento
  float c0r=0,  c1r=0.1507,    c0c=1,  c1c=0; 
  float d1r=-0.83551,   d1c=-1;
...
void setup(){
  Serial.begin(9600); //Se incia la comnicacion por el puerto serial a una tasa de transferencia de 9600 baudios.
  pinMode(13, OUTPUT);
}
...
void loop(){
  if (Serial.available()>0){ //Cada que el Arduino recive un dato por el puerto serial, lee la variable de control y la envia al HMI.
    ...
    SystemOutput = 1033209/pow(R2*(1024/(SystemOutput)-1),1.37265217541467); //Linealizacion del voltaje en Lux
    Control();
...
void Control(){
  if (digitalRead(13)) digitalWrite(13,LOW);  // El led parpadea cada que se ejecuta la estructura
...
float ModelReference(float Input){
